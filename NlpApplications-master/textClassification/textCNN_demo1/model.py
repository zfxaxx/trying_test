...
class TextCNN(nn.Module):
    ...
    def __init__(self, vocab_size, embedding_dim, output_size, filter_num=100, kernel_list=(3, 4, 5), dropout=0.5):
        ...
        self.embedding_kk = nn.Embedding(vocab_size, embedding_dim)
    ...
    def forward(self, x):
        x = self.embedding_kk(x)  # [128, 50, 200] (batch, seq_len, embedding_dim)
...
# cnn, optimizer = amp.initialize(cnn, optimizer, opt_level="O1")
...
def train(model, opt, loss_function):
    ...
    for x_batch, y_batch in dataloader.get_batch(X_train, y_train):
        ...
        avg_acc.append(acc.item())  # 记录该批次正确率
        # 使用损失函数计算损失值，预测值要放在前
        ...
        pred_adv = model(x_batch)        # 模型预测
        loss_adv = loss_function(pred_adv, y_batch)
